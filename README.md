 Welcome to my GitHub Page 👋

Hi, I'm **Arjun Lakhanpal**, an Electronics and Communication Engineering student at BPIT, GGSIPU Delhi-on a mission to bridge real-world challenges and technological innovation.

My core interests lie in **AI**, **machine learning**, **image processing**, and **research-driven engineering**. I’m passionate about building impactful, research-backed solutions using Python, TensorFlow, and other cutting-edge technologies.

Beyond development, I actively share my journey through teaching, open-source contributions, and collaborative initiatives. I’m also a **course creator on Udemy** and YouTuber at **365code.in**.

Currently exploring Large Language Models (LLMs) and the science behind intelligent systems 🧠⚙️

---

## 🌟 Highlights

- 📚 **Udemy Instructor:** *Machine Learning in Hindi: Beginner to Master*  
- 🎓 **ECE + Minor in Computer Science** | BPIT, GGSIPU  
- 🔋 Contributor to battery simulation libraries (LiSimPack with PyBaMM)  
- 🐍 Proficient in Python, TensorFlow, PyTorch, OpenCV  

---

## 📡 What I Love

- AI research & neural networks  
- Biomedical imaging & scientific computing  
- Sharing knowledge & public speaking  

---

## 📬 Let's Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-blue?style=flat-square&logo=linkedin)](https://www.linkedin.com/in/arjunlakhanpall)  
[![Portfolio](https://img.shields.io/badge/Portfolio-green?style=flat-square&logo=google-chrome)](https://about.me/arjunlakhanpal)  
[![YouTube](https://img.shields.io/badge/YouTube-red?style=flat-square&logo=youtube)](https://www.youtube.com/@365codein)

---

> _“Learning never exhausts the mind – especially when curiosity leads.”_
